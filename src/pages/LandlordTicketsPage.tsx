...
export default function LandlordTicketsPage() {
    ...
    const fetchTickets = async () => {
        ...
        const { data, error } = await supabase
            .from('tickets')
            .select('*, properties:property_id(title), profiles:seeker_id(full_name)')
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel('ticket_messages_changes_landlord')
            .on(
                'postgres_changes',
                {
                    event: 'INSERT',
        ...
        <div className="bg-background text-headline min-h-screen animate-[fadeIn_0.25s_ease-out]">
            ...
            <main className="max-w-6xl mx-auto px-6 py-8">
                {activeTicket ? (
                    ...
                    <div className="bg-background border-2 border-headline rounded-sm shadow-[4px_4px_0_var(--color-secondary/10)] overflow-hidden flex flex-col h-[600px] animate-[fadeIn_0.2s_ease-out]">
                        ...
                        <div className="bg-secondary/5 border-b border-secondary/10 p-4 flex items-center gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-paragraph flex flex-wrap items-center gap-2">
                                    ...
                                    <span className="truncate">{activeTicket.profiles?.full_name || 'Seeker'}</span>
                                    <span className="w-1 h-1 rounded-full bg-secondary/30 hidden sm:block"></span>
                                    <span className="truncate">{activeTicket.properties?.title || 'General Inquiry'}</span>
                                </p>
                            </div>
                            {activeTicket.status !== 'resolved' && (
                                <button onClick={handleResolveTicket} className="text-xs font-bold border border-tertiary text-tertiary hover:bg-tertiary/10 transition-colors px-3 py-1.5 rounded-sm whitespace-nowrap hidden sm:block">
                                    Mark Resolved
                                </button>
                            )}
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-secondary/5">
                            {messages.map(msg => {
                                ...
                                const senderName = msg.profiles?.full_name || 'Seeker';
                    ...
                    <>
                        {/* ── Tabs ── */}
                        <div className="flex gap-4 mb-6 border-b border-secondary/10 overflow-x-auto custom-scrollbar">
                        ...
                        {loading ? (
                            ...
                            <div className="text-center py-20 border border-secondary/10 border-dashed rounded-sm bg-secondary/5">
                                ...
                                <p className="text-xs text-paragraph mb-0">You don't have any support tickets assigned to you.</p>
                            </div>
                        ) : (
                                        ...
                                        >
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-paragraph/80 max-w-xl truncate mt-1.5">
                                                        From: <span className="font-semibold text-headline">{ticket.profiles?.full_name || 'Seeker'}</span> • Click to view thread messages
                                                    </p>
                                                </div>
                                            </div>
