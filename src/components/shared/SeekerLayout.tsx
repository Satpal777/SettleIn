...
import { LayoutDashboard, Search, Heart, CalendarDays, CheckSquare, MessageSquare, LogOut, Layers, FileClock, Sun, Moon } from 'lucide-react'
...
const NAV_ITEMS = (hasMovedIn: boolean) => [
    ...
    { label: hasMovedIn ? 'Resident Portal' : 'Move-in Tasks', href: '/seeker/move-in', icon: CheckSquare },
...
export default function SeekerLayout() {
    ...
    const [hasMovedIn, setHasMovedIn] = useState(false)

    useEffect(() => {
        if (!user) return
        async function checkMoveIn() {
            ...
            <aside className="w-64 border-r border-secondary/10 bg-background flex-col shrink-0 hidden md:flex sticky top-0 h-screen">
                ...
                <div className="p-4 border-t border-secondary/10 flex flex-col gap-4">
                    <div className="flex items-center gap-3 px-3">
                        <div className="w-8 h-8 rounded-full bg-highlight/20 text-highlight flex items-center justify-center font-bold text-xs shrink-0 border-2 border-highlight/30">
                            {name.charAt(0).toUpperCase()}
                        </div>
                        <div className="truncate">
                            <p className="text-xs font-bold text-headline truncate">{name}</p>
                            <p className="text-[10px] text-paragraph uppercase tracking-widest font-semibold">{hasMovedIn ? 'Resident' : 'Seeker'}</p>
                        </div>
                    </div>
